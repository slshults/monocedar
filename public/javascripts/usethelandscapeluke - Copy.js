...
ath.intl = {
	...
	en_us: {
		message: 'For the best experience, rotate your device to landscape mode. <br/>And remember, pinch-to-zoom is your friend %icon.',
...
// default options
ath.defaults = {
	appID: 'org.cubic.uselandscape',		// local storage name (no need to change)
...
//	if ( localStorage.getItem('useLandscape') ) {
//	this.optOut();
	}
...
ath.Class.prototype = {
	...
	show: function (force) {
		...
		// try to get the highest resolution landscape icon
		if ( !this.landscapeIcon ) {
			if ( ath.OS == 'ios' ) {
				this.landscapeIcon = document.querySelector('head link[rel^=landscape-icon][sizes="152x89"],head link[rel^=landscape-icon][sizes="144x84"],head link[rel^=landscape-icon][sizes="120x70"],head link[rel^=landscape-icon][sizes="114x67"],head link[rel^=landscape-icon]');
			} else {
				this.landscapeIcon = document.querySelector('head link[rel^="landscape icon"][sizes="196x157"],head link[rel^=landscape-icon]');
			}
		}
		...
		// add the landscape icon
		if ( this.options.icon && this.landscapeIcon ) {
			this.element.className += ' ath-icon';
			this.img = document.createElement('img');
			this.img.className = 'ath-landscape-icon';
			this.img.addEventListener('load', this, false);
			this.img.addEventListener('error', this, false);

			this.img.src = this.landscapeIcon.href;
...
// expose to the world
window.useTheLandscapeLuke = ath;
